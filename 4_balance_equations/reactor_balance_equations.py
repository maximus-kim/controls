...
def vessel(x,t,q,qf,Caf,Tf):
    ...
    # States (3) stores in matrix x:
    # Volume (L) (actually from mass balance where density is constant)
    V = x[0]
    # Concentration of A (mol/L) (from species balance)
    Ca = x[1]
    # Temperature (K) (from energy balance)
    T = x[2]

    # Parameters:
    # Reaction (not necessary as reaction term is = 0)
    rA = 0.0

    # Mass balance: volume derivative
    # has to be computed for the proceeding equations
    dVdt = qf - q

    # Species balance: concentration derivative (molar balance n = c*V)
...
plt.savefig('Transient_Balance_Eq_Plots.png')
plt.show()
